...
import { CCard, CCardHeader, CCardBody, CTable, CButton, CTableHeaderCell, CTableHead, CTableBody, CTableDataCell, CTableRow, CPagination, CPaginationItem } from '@coreui/react';
import './Allusers.css';
...
const Subcontractor = () => {
    ...
    const fetchSubcontractorUsers = async () => {
        try {
            const query = `?page=${page}&limit=${limit}&role=subcontractor`
            ...
            if (res.success) {
                ...
                toast.error('Failed to fetch subcontractor users', toastStyle)
            }
        } catch (err) {
            console.error('API Error:', err);
            toast.error(err.response?.data?.message || 'Failed to fetch subcontractor users', toastStyle)
        }
    }

    useEffect(() => {
        fetchSubcontractorUsers();
    }, [page, limit]);
